    ...
    <div className="max-w-[1440px] mx-auto min-h-[calc(100vh-80px)] flex border-t border-border bg-surface animate-in fade-in duration-500">
          ...
          {conversations.map(conv => (
            <div 
              ...
              className={`p-6 border-b border-border cursor-pointer transition-all flex items-center gap-4 relative group ${
                conv.active ? 'bg-surface-alt' : 'bg-surface hover:bg-surface-alt'
              }`}
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex justify-between items-baseline mb-1">
                  <h4 className={`text-sm truncate font-sans ${conv.active ? 'font-black text-text-primary' : 'font-bold text-text-secondary'}`}>{conv.name}</h4>
                  <span className="text-[9px] font-bold text-text-secondary uppercase">{conv.time}</span>
                </div>
                <p className={`text-xs truncate ${conv.active ? 'text-text-secondary font-medium' : 'text-text-secondary'}`}>{conv.subject}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <main className="flex-1 p-10 md:p-16 overflow-y-auto bg-surface-alt">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="mb-12">
            <h3 className="text-xs font-bold text-text-primary mb-4">Focus Areas</h3>
            <div className="flex flex-wrap gap-3">
              {['Digital Forensics', 'Penetration Testing', 'Malware Triage'].map(tag => (
                <span key={tag} className="px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-widest bg-surface-alt text-text-secondary border border-border">
                  {tag}
                </span>
              ))}
            </div>
          ...
          <div className="flex justify-end gap-6 pt-8 border-t border-border">
            <button className="px-10 py-3 border border-border text-text-secondary font-bold text-sm rounded-xl hover:bg-red-50 hover:text-red-600 hover:border-red-200 transition-all active:scale-95">
              Decline
            </button>
